...
- Gene Panel 後台 shell 與常用 API 測試範例有哪些
- 哪些方法適合覆寫，哪些不應該亂放邏輯
- 新人今天要查一個函式時，應該先去哪裡看
...
### [api_route_reference.md](api_route_reference.md)
- 用途：快速查 `/api/@module/@method` 的 route contract、module / method 命名規則、class / method 映射方式與常見 request 範例。
- 適合在已經知道要走 Reaction API route，但需要確認 path naming、request shape 與對應 action 時使用。
- 最適合的讀者：正在串接、驗證或設計 backend API route 的工程師。

### [Gene Panel.md](Gene%20Panel.md)
- 用途：快速查 Gene Panel 的定位、與 F3CMS Reaction 的協作方式、後台 API 測試重點端點與常見返回值範例。
- 適合在已經知道要檢查 Gene Panel 後台 shell、`www/backend/`、`www/tmpls/` 或後台 AJAX route 行為時使用。
- 最適合的讀者：正在串接或驗證 Gene Panel 後台的工程師。

### [smoke_s_layer_reference.md](smoke_s_layer_reference.md)
- 用途：快速查 `www/tests/index.php <module>/<surface>/<contract>` 的正式 contract、path grammar、guard 規則、error vocabulary 與驗證命令。
- 適合在已經知道要走 Smoke S Layer 正式入口，但需要確認 path 解析、module discovery 或 guard 行為時使用。
- 最適合的讀者：正在擴充、驗證或 review module-owned smoke 的工程師。
...
- `/api/@module/@method` 會打到哪個 Reaction class / method，先看 [api_route_reference.md](api_route_reference.md)
- Gene Panel 後台會呼叫哪些登入、清單、get、save、get_opts API，先看 [Gene Panel.md](Gene%20Panel.md)
- `www/tests/index.php mobile/request/create_or_ensure` 這類 smoke path 會打到哪個 owner smoke、會被哪些 guard 阻擋，先看 [smoke_s_layer_reference.md](smoke_s_layer_reference.md)
...
6. 若要串接或驗證 `/api/...` backend route，再補看 [api_route_reference.md](api_route_reference.md)
7. 若要串接或驗證 Gene Panel 後台 shell 與 API，再補看 [Gene Panel.md](Gene%20Panel.md)
8. 若要串接或驗證 FORKS 的正式 smoke 入口，再補看 [smoke_s_layer_reference.md](smoke_s_layer_reference.md)
...
4. 依需求回查 [feed_reference.md](feed_reference.md)、[reaction_reference.md](reaction_reference.md)、[outfit_reference.md](outfit_reference.md)、[api_route_reference.md](api_route_reference.md)、[Gene Panel.md](Gene%20Panel.md)
5. 若變更涉及 module-owned smoke 或 `www/tests/index.php`，補看 [smoke_s_layer_reference.md](smoke_s_layer_reference.md)
...
3. 若 review 涉及 backend API path contract 或 Gene Panel 後台行為，再補看 [api_route_reference.md](api_route_reference.md) 與 [Gene Panel.md](Gene%20Panel.md)
4. 若 review 涉及 FORKS 的 Smoke S Layer contract，再補看 [smoke_s_layer_reference.md](smoke_s_layer_reference.md)
