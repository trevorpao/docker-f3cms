...
由內容 module owner 先建立或載入 `{entity}_seen` truth，再交由 `Duty` kit 執行 task evaluate、done 與 reward writeback 協調的可重用 completion contract。它不是單一內容 module 專屬流程，而是一條可被不同 seen-driven task 共用的主線。
...
### API Route Contract
F3CMS backend API 的穩定 path 與映射規則，固定採 `/api/@module/@method`。其中 `module` 對應 `r{Module}`，`method` 對應 `do_{method}`；公開 path 使用小寫 module 與 snake_case method。

### Response Envelope
Reaction route 對外回傳的標準 JSON shape，核心欄位為 `code`、`data`、`csrf`。判讀 route 成功或失敗時，應先看 body 內的 `code`，而不是只看 HTTP status。
...
### prerequisite_unmet
Seen-target completion 中的 prerequisite-specific skip reason。表示 dependency 已成功 resolve 到 duty 與 member task row，但 `expected_status` 尚未成立，因此 task 目前不可完成。

### prerequisite_unresolvable
Seen-target completion 中的 prerequisite-specific skip reason。表示 prerequisite dependency identifier 已提供，但無法 resolve 到 duty，或 member 缺少對應 dependency task row，因此 task 目前不可完成。

### task_unreachable
保留中的 generic unreachable vocabulary。它可用於未來非-prerequisite achievable gating 的通用命名，但在 SeenTargetTaskCompletion current spec 中，不再作為 prerequisite domain 的 runtime skip reason。
