...
tests_smoke_run(function () {
    ...
    $metPressId = 0;
    $dependentPressId = 0;
    $unmetDutySlug = 'seen-target-prerequisite-or-unmet-' . uniqid();
    $metDutySlug = 'seen-target-prerequisite-or-met-' . uniqid();
    $dependentDutySlug = 'seen-target-prerequisite-or-dependent-' . uniqid();
    $unmetTaskTemplateSlug = 'prerequisite-or-unmet-template-' . uniqid();
    $metTaskTemplateSlug = 'prerequisite-or-met-template-' . uniqid();
    $dependentTaskTemplateSlug = 'prerequisite-or-dependent-template-' . uniqid();

    $staleDutyIds = mh()->query("SELECT id FROM tbl_duty WHERE slug LIKE 'seen-target-prerequisite-or-unmet-%' OR slug LIKE 'seen-target-prerequisite-or-met-%' OR slug LIKE 'seen-target-prerequisite-or-dependent-%'")->fetchAll(\PDO::FETCH_COLUMN);
    ...
    mh()->insert('tbl_member', [
        'display_name' => 'Seen Target Prerequisite OR Member',
        'status' => 'Enabled',
    ...
    foreach ([
        'unmet' => 'seen-target-prerequisite-or-unmet-press-' . uniqid(),
        'met' => 'seen-target-prerequisite-or-met-press-' . uniqid(),
        'dependent' => 'seen-target-prerequisite-or-dependent-press-' . uniqid(),
    ] as $key => $pressSlug) {
        mh()->insert('tbl_press', [
            'slug' => $pressSlug,
            'status' => 'Published',
    ...
    $unmetPayload['task_template']['slug'] = $unmetTaskTemplateSlug;
    ...
    $metPayload = $payload;
    $metPayload['task_template']['slug'] = $metTaskTemplateSlug;
    ...
    $dependentPayload['task_template']['prerequisite'] = [
        ...
        'tasks' => [
            ...
            [
                'task_template_slug' => $metTaskTemplateSlug,
                'expected_status' => \F3CMS\fTask::ST_DONE,
            ],
        ],
    ];
    ...
    try {
        ...
        if ($unmetTaskId <= 0 || $metTaskId <= 0 || $dependentTaskId <= 0) {
            throw new \RuntimeException('Expected OR prerequisite smoke to create unmet, met, and dependent tasks.');
        }
        ...
        if (in_array($dependentDutyId, $beforeVisibleDutyIds, true)) {
            throw new \RuntimeException('Expected OR prerequisite dependent task to stay hidden before any prerequisite is done.');
        }
        ...
        if (1 !== count($metResult['completed_tasks'] ?? []) || $metDutyId !== (int) (($metResult['completed_tasks'][0]['duty_id'] ?? 0))) {
            throw new \RuntimeException('Expected task_template_slug prerequisite target to complete its own prerequisite task first.');
        }
        ...
        if (!in_array($dependentDutyId, $afterVisibleDutyIds, true)) {
            throw new \RuntimeException('Expected OR prerequisite dependent task to become visible after any satisfiable prerequisite is done.');
        }
        ...
        if (empty($dependentCompleted) || \F3CMS\fTask::ST_DONE !== ($dependentCompleted['status'] ?? null)) {
            throw new \RuntimeException('Expected OR prerequisite dependent task to complete once task_template_slug prerequisite is satisfied.');
        }
        ...
        if (\F3CMS\fTask::ST_DONE !== ($dependentTask['status'] ?? null)) {
            throw new \RuntimeException('Expected dependent task row to persist as Done after OR prerequisite completion path.');
        }

        if (200 !== (int) ($account['balance'] ?? -1) || 2 !== $taskDoneLogCount || 2 !== $accountLogCount) {
            throw new \RuntimeException('Expected OR prerequisite completion path to write both prerequisite and dependent rewards exactly once.');
        }
