...
tests_smoke_run(function () {
    ...
    $taskId = 0;
    $pressId = 0;
    $slug = 'seen-target-repeat-short-circuit-' . uniqid();
    $pressSlug = 'seen-target-repeat-press-' . uniqid();

    $staleDutyIds = mh()->query("SELECT id FROM tbl_duty WHERE slug LIKE 'seen-target-repeat-short-circuit-%'")->fetchAll(\PDO::FETCH_COLUMN);
    ...
    mh()->insert('tbl_member', [
        'display_name' => 'Seen Target Repeat Short Circuit Smoke Member',
        'status' => 'Enabled',
    ...
    try {
        ...
        if ($taskId <= 0 || \F3CMS\fTask::ST_NEW !== ($before['status'] ?? null)) {
            throw new \RuntimeException('Expected repeat short-circuit smoke to create a New task before seen completion.');
        }

        \F3CMS\rPress::completeSeenForMember($memberId, [
            'id' => $pressId,
            'source' => 'rPress',
        ]);
        $repeat = \F3CMS\rPress::completeSeenForMember($memberId, [
            'slug' => $pressSlug,
            'source' => 'rPress',
        ]);
        ...
        if (\F3CMS\fTask::ST_DONE !== ($afterTask['status'] ?? null)) {
            throw new \RuntimeException('Expected task to remain Done on repeat seen completion.');
        }

        if (100 !== (int) ($account['balance'] ?? -1)) {
            throw new \RuntimeException('Expected reward balance to remain 100 on repeat seen completion.');
        }

        if (0 !== count($repeat['completed_tasks'] ?? [])) {
            throw new \RuntimeException('Expected repeat seen completion to avoid new completed_tasks entries.');
        }

        if (1 !== count($repeat['short_circuited_tasks'] ?? [])) {
            throw new \RuntimeException('Expected repeat seen completion to return exactly one short-circuited task.');
        }

        if ('already_completed' !== (($repeat['short_circuited_tasks'][0]['reason'] ?? null))) {
            throw new \RuntimeException('Expected repeat seen short-circuit reason to be already_completed.');
        }

        if (!empty($repeat['skipped_tasks'] ?? [])) {
            throw new \RuntimeException('Expected repeat seen completion to avoid skipped_tasks when the contract is a pure already_completed short-circuit.');
        }

        if (!empty($repeat['seen']['_created'])) {
            throw new \RuntimeException('Expected repeat seen completion to reuse existing press_seen truth.');
        }
        ...
        if (1 !== $pressSeenCount || 0 !== $memberSeenCount || 1 !== $taskLogCount || 1 !== $accountLogCount) {
            throw new \RuntimeException('Expected repeat seen short-circuit to preserve press_seen truth ownership and single-row task/account logs.');
        }
