...
tests_smoke_run(function () {
    ...
    $taskId = 0;
    $pressId = 0;
    $slug = 'seen-target-press-reward-' . uniqid();
    $pressSlug = 'seen-target-press-' . uniqid();

    $staleDutyIds = mh()->query("SELECT id FROM tbl_duty WHERE slug LIKE 'seen-target-press-reward-%'")->fetchAll(\PDO::FETCH_COLUMN);
    ...
    mh()->insert('tbl_member', [
        'display_name' => 'Seen Target Press Reward Smoke Member',
        'status' => 'Enabled',
    ...
    try {
        ...
        if ($taskId <= 0 || \F3CMS\fTask::ST_NEW !== ($before['status'] ?? null)) {
            throw new \RuntimeException('Expected SeenTargetTaskCompletion press reward smoke to create a New task before seen completion.');
        }
        ...
        if (\F3CMS\fTask::ST_DONE !== ($afterTask['status'] ?? null)) {
            throw new \RuntimeException('Expected task to become Done after press seen completion.');
        }

        if (100 !== (int) ($account['balance'] ?? -1)) {
            throw new \RuntimeException('Expected member reward balance to become 100 after press seen completion.');
        }

        if (1 !== count($first['completed_tasks'] ?? [])) {
            throw new \RuntimeException('Expected first press seen completion to complete exactly one task.');
        }

        if (0 !== count($second['completed_tasks'] ?? [])) {
            throw new \RuntimeException('Expected repeat press seen completion to stay idempotent.');
        }

        if (0 !== count($first['short_circuited_tasks'] ?? [])) {
            throw new \RuntimeException('Expected first press seen completion to avoid short-circuit results before task completion.');
        }

        if (1 !== count($second['short_circuited_tasks'] ?? [])) {
            throw new \RuntimeException('Expected repeat press seen completion to report exactly one already_completed short-circuit task.');
        }

        if ('already_completed' !== (($second['short_circuited_tasks'][0]['reason'] ?? null))) {
            throw new \RuntimeException('Expected repeat press seen short-circuit reason to be already_completed.');
        }

        if (empty($first['seen']['_created']) || !empty($second['seen']['_created'])) {
            throw new \RuntimeException('Expected press_seen truth to be created once and reused across press requests.');
        }
        ...
        if (1 !== $pressSeenCount || 0 !== $memberSeenCount || 1 !== $taskLogCount || 1 !== $accountLogCount) {
            throw new \RuntimeException('Expected press_seen to own truth writes and preserve single-row task/account logs after repeat press completion.');
        }
