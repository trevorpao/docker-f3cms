...
tests_smoke_run(function () {
    ...
    $dependentTaskId = 0;
    $dependentPressId = 0;
    $missingDutySlug = 'seen-target-prerequisite-missing-duty-' . uniqid();
    $dependentDutySlug = 'seen-target-prerequisite-unresolvable-dependent-' . uniqid();

    $staleDutyIds = mh()->query("SELECT id FROM tbl_duty WHERE slug LIKE 'seen-target-prerequisite-unresolvable-dependent-%'")->fetchAll(\PDO::FETCH_COLUMN);
    ...
    mh()->insert('tbl_member', [
        'display_name' => 'Seen Target Unresolvable Dependency Member',
        'status' => 'Enabled',
    ...
    mh()->insert('tbl_press', [
        'slug' => 'seen-target-prerequisite-unresolvable-dependent-press-' . uniqid(),
        'status' => 'Published',
        'insert_ts' => date('Y-m-d H:i:s'),
        'insert_user' => 1,
        'last_ts' => date('Y-m-d H:i:s'),
    ...
    try {
        ...
        if ($dependentTaskId <= 0) {
            throw new \RuntimeException('Expected unresolvable dependency smoke to create dependent task.');
        }
        ...
        if (1 !== count($allPending)) {
            throw new \RuntimeException('Expected raw pending query to keep the dependent task with unresolvable prerequisite.');
        }

        if (!empty($visiblePending)) {
            throw new \RuntimeException('Expected pending query to hide task with unresolvable prerequisite dependency.');
        }
        ...
        if (empty($result['seen']['_created'])) {
            throw new \RuntimeException('Expected unresolvable dependency completion path to preserve seen truth creation.');
        }

        if (!empty($result['completed_tasks'] ?? [])) {
            throw new \RuntimeException('Expected unresolvable dependency completion path to avoid completed_tasks.');
        }
        ...
        if (empty($dependentSkip) || \F3CMS\fDuty::SK_PREREQUISITE_UNRESOLVABLE !== ($dependentSkip['reason'] ?? null)) {
            throw new \RuntimeException('Expected unresolvable dependency dependent task to be skipped as prerequisite_unresolvable.');
        }

        if (\F3CMS\fTask::ST_NEW !== ($afterDependentTask['status'] ?? null)) {
            throw new \RuntimeException('Expected unresolvable dependency path to preserve New status for dependent task.');
        }

        if (0 !== (int) ($account['balance'] ?? -1)) {
            throw new \RuntimeException('Expected unresolvable dependency path to avoid reward writes.');
        }
        ...
        if (1 !== $dependentPressSeenCount || 0 !== $memberSeenCount || 0 !== $taskLogCount || 0 !== $accountLogCount) {
            throw new \RuntimeException('Expected unresolvable dependency path to keep only incoming seen truth and avoid reward writes.');
        }
