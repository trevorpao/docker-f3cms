...
tests_smoke_run(function () {
    ...
    $dependentPressId = 0;
    $prerequisiteSlug = 'seen-target-prerequisite-template-base-' . uniqid();
    $dependentSlug = 'seen-target-prerequisite-template-dependent-' . uniqid();
    $prerequisiteTaskTemplateSlug = 'seen-target-prerequisite-template-task-' . uniqid();
    $dependentTaskTemplateSlug = 'seen-target-prerequisite-template-dependent-task-' . uniqid();

    $staleDutyIds = mh()->query("SELECT id FROM tbl_duty WHERE slug LIKE 'seen-target-prerequisite-template-base-%' OR slug LIKE 'seen-target-prerequisite-template-dependent-%'")->fetchAll(\PDO::FETCH_COLUMN);
    ...
    mh()->insert('tbl_member', [
        'display_name' => 'Seen Target Prerequisite Template Smoke Member',
        'status' => 'Enabled',
    ...
    mh()->insert('tbl_press', [
        'slug' => 'seen-target-prerequisite-template-press-' . uniqid(),
        'status' => 'Published',
        'insert_ts' => date('Y-m-d H:i:s'),
        'insert_user' => 1,
        'last_ts' => date('Y-m-d H:i:s'),
    ...
    mh()->insert('tbl_press', [
        'slug' => 'seen-target-prerequisite-template-dependent-press-' . uniqid(),
        'status' => 'Published',
        'insert_ts' => date('Y-m-d H:i:s'),
        'insert_user' => 1,
        'last_ts' => date('Y-m-d H:i:s'),
    ...
    $prerequisitePayload['task_template']['slug'] = $prerequisiteTaskTemplateSlug;
    ...
    try {
        ...
        if ($prerequisiteTaskId <= 0 || $dependentTaskId <= 0) {
            throw new \RuntimeException('Expected task_template_slug AND prerequisite smoke to create both prerequisite and dependent tasks.');
        }
        ...
        if (2 !== count($allPending)) {
            throw new \RuntimeException('Expected raw pending query to keep both task_template_slug prerequisite tasks.');
        }

        if (1 !== count($visiblePending) || $prerequisiteDutyId !== (int) ($visiblePending[0]['duty_id'] ?? 0)) {
            throw new \RuntimeException('Expected pending query to exclude unmet AND task_template_slug prerequisite task.');
        }
        ...
        if (empty($result['seen']['_created'])) {
            throw new \RuntimeException('Expected unmet AND task_template_slug prerequisite path to preserve seen truth creation.');
        }

        if (!empty($result['completed_tasks'] ?? [])) {
            throw new \RuntimeException('Expected unmet AND task_template_slug prerequisite path to avoid completed_tasks.');
        }
        ...
        if (empty($dependentSkip) || \F3CMS\fDuty::SK_PREREQUISITE_UNMET !== ($dependentSkip['reason'] ?? null)) {
            throw new \RuntimeException('Expected unmet AND task_template_slug dependent task to be skipped as prerequisite_unmet.');
        }

        if (\F3CMS\fTask::ST_NEW !== ($afterPrerequisiteTask['status'] ?? null) || \F3CMS\fTask::ST_NEW !== ($afterDependentTask['status'] ?? null)) {
            throw new \RuntimeException('Expected unmet AND task_template_slug path to preserve New status for both tasks.');
        }

        if (0 !== (int) ($account['balance'] ?? -1)) {
            throw new \RuntimeException('Expected unmet AND task_template_slug path to avoid reward writes.');
        }
        ...
        if (1 !== $dependentPressSeenCount || 0 !== $memberSeenCount || 0 !== $taskLogCount || 0 !== $accountLogCount) {
            throw new \RuntimeException('Expected unmet AND task_template_slug path to keep only incoming seen truth and avoid reward writes.');
        }
