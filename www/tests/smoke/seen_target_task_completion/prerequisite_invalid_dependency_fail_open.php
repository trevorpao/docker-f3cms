...
tests_smoke_run(function () {
    ...
    $dependentPressId = 0;
    $prerequisiteSlug = 'seen-target-prerequisite-invalid-dependency-base-' . uniqid();
    $dependentSlug = 'seen-target-prerequisite-invalid-dependency-dependent-' . uniqid();

    $staleDutyIds = mh()->query("SELECT id FROM tbl_duty WHERE slug LIKE 'seen-target-prerequisite-invalid-dependency-base-%' OR slug LIKE 'seen-target-prerequisite-invalid-dependency-dependent-%'")->fetchAll(\PDO::FETCH_COLUMN);
    ...
    mh()->insert('tbl_member', [
        'display_name' => 'Seen Target Invalid Dependency Member',
        'status' => 'Enabled',
    ...
    mh()->insert('tbl_press', [
        'slug' => 'seen-target-prerequisite-invalid-dependency-base-press-' . uniqid(),
        'status' => 'Published',
        'insert_ts' => date('Y-m-d H:i:s'),
        'insert_user' => 1,
        'last_ts' => date('Y-m-d H:i:s'),
    ...
    mh()->insert('tbl_press', [
        'slug' => 'seen-target-prerequisite-invalid-dependency-dependent-press-' . uniqid(),
        'status' => 'Published',
        'insert_ts' => date('Y-m-d H:i:s'),
        'insert_user' => 1,
        'last_ts' => date('Y-m-d H:i:s'),
    ...
    $prerequisitePayload['task_template']['slug'] = 'prerequisite-invalid-dependency-base';
    ...
    $dependentPayload['task_template']['slug'] = 'prerequisite-invalid-dependency-dependent';
    ...
    try {
        ...
        if ($prerequisiteTaskId <= 0 || $dependentTaskId <= 0) {
            throw new \RuntimeException('Expected invalid dependency fail-open smoke to create both prerequisite and dependent tasks.');
        }
        ...
        if (!in_array($dependentDutyId, $visiblePendingDutyIds, true)) {
            throw new \RuntimeException('Expected invalid prerequisite dependency payload to fail-open and keep dependent task visible.');
        }
        ...
        if (empty($dependentCompleted) || \F3CMS\fTask::ST_DONE !== ($dependentCompleted['status'] ?? null)) {
            throw new \RuntimeException('Expected invalid prerequisite dependency payload to fail-open and allow dependent task completion.');
        }

        if (\F3CMS\fTask::ST_NEW !== ($afterPrerequisiteTask['status'] ?? null) || \F3CMS\fTask::ST_DONE !== ($afterDependentTask['status'] ?? null)) {
            throw new \RuntimeException('Expected invalid prerequisite dependency fail-open path to preserve prerequisite task and complete only dependent task.');
        }

        if (100 !== (int) ($account['balance'] ?? -1)) {
            throw new \RuntimeException('Expected invalid prerequisite dependency fail-open path to write one reward for dependent task.');
        }
