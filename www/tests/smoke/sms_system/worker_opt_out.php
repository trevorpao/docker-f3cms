...
class SmsSystemOptOutSmokeProvider implements SmsProviderInterface
{
...
tests_smoke_run(function () {
    ...
    SmsSystemOptOutSmokeProvider::reset();
    ...
    try {
        mh()->insert('tbl_member', [
            'display_name' => 'SMS Opt-out Smoke Member',
            'status' => 'Enabled',
        ...
        mh()->insert('tbl_phonebook', [
            'member_id' => $memberId,
            'title' => 'SMS Opt-out Phonebook ' . uniqid(),
            'remark' => 'Smoke',
            'status' => 'Enabled',
            'insert_ts' => date('Y-m-d H:i:s'),
            'insert_user' => 1,
        ...
        $fakeSender = new \F3CMS\smSender([
            'mitake' => new SmsSystemOptOutSmokeProvider(),
        ], 'mitake', '+886', null);
        ...
        if (1 !== count($results)) {
            throw new \RuntimeException('Expected worker opt-out smoke to process exactly one campaign log.');
        }

        $result = $results[0];
        if (($result['status'] ?? null) !== \F3CMS\fCampaign::LOG_FAILED || ($result['error_message'] ?? null) !== 'Opt-out Number') {
            throw new \RuntimeException('Expected opt-out worker path to return Failed with Opt-out Number.');
        }

        if (($log['status'] ?? null) !== \F3CMS\fCampaign::LOG_FAILED || ($log['error_message'] ?? null) !== 'Opt-out Number') {
            throw new \RuntimeException('Expected campaign log to converge to Failed with Opt-out Number.');
        }

        if (empty($log['attempt_ts'])) {
            throw new \RuntimeException('Expected opt-out worker path to still write attempt_ts.');
        }

        if (!empty($log['sent_ts']) || !empty($log['provider_message_id'])) {
            throw new \RuntimeException('Expected opt-out worker path to keep sent_ts and provider_message_id empty.');
        }

        if (SmsSystemOptOutSmokeProvider::callCount() !== 0) {
            throw new \RuntimeException('Expected opt-out worker guardrail to stop before provider dispatch.');
        }
        ...
        if ((int) ($campaignAfter['sent_count'] ?? 0) !== 0 || (int) ($campaignAfter['failed_count'] ?? 0) !== 1 || ($campaignAfter['status'] ?? null) !== \F3CMS\fCampaign::ST_FAILED) {
            throw new \RuntimeException('Expected campaign summary to converge to Failed with failed_count=1 for opt-out.');
        }

        if (!empty($mobileAfter['last_sent_ts'])) {
            throw new \RuntimeException('Expected opt-out worker path to leave tbl_mobile.last_sent_ts empty.');
        }
        ...
        return [
            ...
            'mobile' => $mobileAfter,
            'provider_calls' => SmsSystemOptOutSmokeProvider::callCount(),
        ];
    } finally {
