...
class SmsSystemProviderFailureSmokeProvider implements SmsProviderInterface
{
...
tests_smoke_run(function () {
    ...
    SmsSystemProviderFailureSmokeProvider::reset();
    ...
    try {
        mh()->insert('tbl_member', [
            'display_name' => 'SMS Provider Failure Smoke Member',
            'status' => 'Enabled',
        ...
        mh()->insert('tbl_phonebook', [
            'member_id' => $memberId,
            'title' => 'SMS Provider Failure Phonebook ' . uniqid(),
            'remark' => 'Smoke',
            'status' => 'Enabled',
            'insert_ts' => date('Y-m-d H:i:s'),
            'insert_user' => 1,
        ...
        $campaign = \F3CMS\fCampaign::createForPhonebook(
            $memberId,
            $phonebookId,
            'Worker provider failure smoke content',
        ...
        $fakeSender = new \F3CMS\smSender([
            'mitake' => new SmsSystemProviderFailureSmokeProvider(),
        ], 'mitake', '+886', null);
        ...
        if (1 !== count($results)) {
            throw new \RuntimeException('Expected worker provider-failure smoke to process exactly one campaign log.');
        }

        $result = $results[0];
        if (($result['status'] ?? null) !== \F3CMS\fCampaign::LOG_FAILED || ($result['error_message'] ?? null) !== 'Provider Error: upstream timeout') {
            throw new \RuntimeException('Expected provider-failure worker path to return Failed with Provider Error: upstream timeout.');
        }

        if (($log['status'] ?? null) !== \F3CMS\fCampaign::LOG_FAILED || ($log['error_message'] ?? null) !== 'Provider Error: upstream timeout') {
            throw new \RuntimeException('Expected campaign log to converge to Failed with Provider Error: upstream timeout.');
        }

        if (empty($log['attempt_ts'])) {
            throw new \RuntimeException('Expected provider-failure worker path to still write attempt_ts.');
        }

        if (!empty($log['sent_ts']) || !empty($log['provider_message_id'])) {
            throw new \RuntimeException('Expected provider-failure worker path to keep sent_ts and provider_message_id empty.');
        }

        if (SmsSystemProviderFailureSmokeProvider::callCount() !== 1) {
            throw new \RuntimeException('Expected provider-failure worker path to call provider exactly once.');
        }
        ...
        if ((int) ($campaignAfter['sent_count'] ?? 0) !== 0 || (int) ($campaignAfter['failed_count'] ?? 0) !== 1 || ($campaignAfter['status'] ?? null) !== \F3CMS\fCampaign::ST_FAILED) {
            throw new \RuntimeException('Expected campaign summary to converge to Failed with failed_count=1 for provider failure.');
        }

        if (!empty($mobileAfter['last_sent_ts'])) {
            throw new \RuntimeException('Expected provider-failure worker path to leave tbl_mobile.last_sent_ts empty.');
        }
        ...
        return [
            ...
            'mobile' => $mobileAfter,
            'provider_calls' => SmsSystemProviderFailureSmokeProvider::callCount(),
        ];
    } finally {
